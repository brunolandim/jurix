...
export function KanbanColumn({ column, onCardClick }: KanbanColumnProps) {
  ...
  return (
    <div className="shrink-0 w-72 rouded rounded-md bg-default-100 flex flex-col max-h-full">
      <div className="p-3 border-b border-default-200">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold">{column.title}</h3>
          <span className="text-xs bg-default-200 px-2 py-1 rounded-full">{column.cases.length}</span>
        </div>
      </div>
      <div
        ref={setNodeRef}
